...
func TestIsPublicNameRejectsUnissuableNames(t *testing.T) {
	public := []string{"mail.example.com", "mx1.example.net", "example.org"}
...
func TestCertificateWantedNamesCoversHostAndOverrides(t *testing.T) {
	...
	shared := &EmailDomain{Domain: "example.org", Enabled: true, MXRecord: "mail.example.org"}
	if err := memory.Create(m.db, "email_domains", shared); err != nil {
		t.Fatalf("create domain: %v", err)
	}
	// A domain with its own mail host does.
	custom := &EmailDomain{Domain: "example.net", Enabled: true, MXRecord: "posta.example.net"}
	if err := memory.Create(m.db, "email_domains", custom); err != nil {
		t.Fatalf("create domain: %v", err)
	}
	// A disabled domain is not served, so it is not certified either.
	disabled := &EmailDomain{Domain: "disabled.example.com", Enabled: false, MXRecord: "posta.disabled.example.com"}
...
func TestCertificateStatusReportsMissingNames(t *testing.T) {
	...
	domain := &EmailDomain{Domain: "example.org", Enabled: true, MXRecord: "posta.example.org"}
	...
	if !containsString(status.Wanted, "posta.example.org") {
		t.Errorf("the wanted set is wrong: %v", status.Wanted)
	}
	if !containsString(status.Missing, "posta.example.org") {
		t.Errorf("a name the certificate does not cover must be reported: %+v", status)
	}
	if containsString(status.Missing, "mx.example.com") {
...
func TestMXHostIsSharedAcrossDomains(t *testing.T) {
	...
	first := &EmailDomain{Domain: "first.example.com", Enabled: true, MXRecord: "mail.first.example.com"}
	second := &EmailDomain{Domain: "second.example.com", Enabled: true, MXRecord: "mail.second.example.com"}
	...
	if got := m.mxHostFor(first); got != "mx.example.com" {
		t.Errorf("all domains should point at the shared mail host, got %q", got)
	}
	if got := m.mxHostFor(second); got != "mx.example.com" {
		t.Errorf("all domains should point at the shared mail host, got %q", got)
	}

	// One certificate name serves every domain.
	names := m.certificateWantedNames()
	if len(names) != 1 || names[0] != "mx.example.com" {
...
func TestExplicitMXOverrideIsKeptAndCertified(t *testing.T) {
	...
	if got := m.mxHostFor(custom); got != "posta.example.net" {
...
func TestWithoutAServerHostnameEachDomainGetsItsOwnName(t *testing.T) {
	...
	domain := &EmailDomain{Domain: "fallback.example.com", Enabled: true, MXRecord: "mail.fallback.example.com"}
	if err := memory.Create(m.db, "email_domains", domain); err != nil {
		t.Fatalf("create domain: %v", err)
	}

	if got := m.mxHostFor(domain); got != "mail.fallback.example.com" {
		t.Errorf("without a usable server hostname the per-domain host is the fallback, got %q", got)
	}
	if names := m.certificateWantedNames(); !containsString(names, "mail.fallback.example.com") {
